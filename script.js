(function () {
  ...
  const ZOOM_CONFIG = {
    ...
    month: 25, quarter: 10, semester: 5, annual: 3
  };
  ...
  function computeTimelineRange() {
    ...
    else if (['annual', 'semester', 'quarter'].includes(config.scale)) {
      earliest.setDate(1); 
  ...
  function createHeader() {
    ...
    if (config.scale === 'hours') {
      ...
      while (current < timelineEndDate) {
        ...
        const blockWidth = daysInMonth * pxPerUnit;

        const block = document.createElement('div');
        block.className = 'day-block';
        block.style.width = blockWidth + 'px';
        block.style.borderRight = '1px solid rgba(0,0,0,0.05)';
        
        const monthName = monthStart.toLocaleString('es-ES', { month: 'long', year: 'numeric' });
        block.innerHTML = `<div class="day-label" style="font-size:12px; text-transform:capitalize;">${monthName}</div>`;
      ...
      for(let i=0; i<totalUnits; i++) {
        ...
        let text = "";
        if (config.scale === 'month') text = d.getDate();
        else if (config.scale === 'fortnight') text = `${d.getDate()}/${d.getMonth()+1}`;
        else text = fmtDateClean(d);

        const isWeekend = d.getDay() === 0 || d.getDay() === 6;
        if(isWeekend) block.style.backgroundColor = 'rgba(0,0,0,0.02)';
  ...
  function renderMenu() {
    ...
    data.forEach(center => {
      ...
      card.innerHTML = `<div class="sidebar-center-header"><div>${center.code} — ${center.name}</div><div style="font-size:11px;opacity:0.7">(${visibleMachines.length}) ${center.expanded?'−':'＋'}</div></div>`;
      ...
      card.querySelector('.sidebar-center-header').onclick = () => { center.expanded = !center.expanded; refreshAll(); };
